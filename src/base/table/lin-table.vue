  ...
  <el-table
    ...
    <el-table-column label="备注" width="200" :show-overflow-tooltip="true">
      <template scope="scope">
        <div style="display: flex; align-items: center; justify-content: space-between;width:135px;">
          <div v-if="!scope.row.editFlag" @click="handleEdit(scope.row)">{{ scope.row.remark }}</div>
    ...
    <el-table-column label="推荐" prop="recommend" v-if="!recommendHidden">
      <template slot-scope="scope">
        <el-switch v-model="scope.row.recommend" active-color="#3963bc" @change="handleRecommend($event, scope.row)"></el-switch>
      </template>
    </el-table-column>
    <!-- 操作列 -->
    <el-table-column label="操作" fixed="right" width="150">
      <template slot-scope="scope">
        <lin-button
          ...
          @click.native.prevent.stop="buttonMethods(item.func, scope.$index, tableData)"></lin-button>
...
export default {
  ...
  methods: {
    ...
    // 单元格编辑
    handleCellEdit: function (row) {
      row.editFlag = true;
      this.tempEditRemark = row.remark
    },
    handleCellSave: function (row) {
      row.editFlag = false;
    },
