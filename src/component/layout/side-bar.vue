  ...
  <div class="app-sidebar">
    ...
    <div style="margin-bottom:50px">
      ...
      <el-menu
        ...
        :default-openeds="idMap.openedMenus"
      ...
      >
        <template v-for="item in sideBarList">
          <el-submenu
            ...
            v-if="item.children"
            :key="idMap.indexs[item.name]"
            :index="idMap.indexs[item.name]"
            popper-class="abc"
          >
            <template slot="title">
            ...
            <template v-for="subItem in item.children">
              <el-submenu
                ...
                :key="idMap.indexs[subItem.name]"
                :index="idMap.indexs[subItem.name]"
                class="subMenuContent"
              >
                <template slot="title">
                ...
                <router-link
                  ...
                  :key="idMap.indexs[grandchildItem.name]"
                  :to="grandchildItem.path"
                  class="circle third"
                >
                  <el-menu-item
                    :index="idMap.indexs[grandchildItem.name]"
              ...
              <router-link :to="subItem.path" :key="subItem.name" class="circle" v-else>
                <el-menu-item :index="idMap.indexs[subItem.name]" style="padding-left: 60px;" class="subMenuContent">
                  {{ subItem.title }}
                </el-menu-item>
              </router-link>
            </template>
  ...
  computed: {
    ...
    defaultActive() {
      for (let i = this.stageInfo.length - 1; i >= 0; i -= 1) {
        if (this.idMap.indexs[this.stageInfo[i].name]) {
          return this.idMap.indexs[this.stageInfo[i].name]
        }
      }
      return ''
    },
    ...
    idMap() {
      ...
      deepTravel(sideBarList, item => {
        mapData.indexs[item.name] = Utils.getRandomStr()
        if (item.title === '权限管理' || item.title === '用户管理' || item.title === 'UI') {
          mapData.openedMenus.push(mapData.indexs[item.name])
        }
      })
